...
def main(event, context):
    ...
    try:
        ...
        if "error" in status:
            ...
            error_messages += "Failed to get Cityworks token. {}\n".format(status)
            # else:
        ...
        if sr == "error":
            ...
            error_messages += "Spatial reference not defined\n"
            # else:
        ...
        if prob_types == "error":
            ...
            error_messages += "Problem types not defined\n"
            # else:
        ...
        for layer in layers:
            ...
            for row in rows.features:
                try:
                    ...
                    error_messages += "Status of updates to {}, ObjectID:{} {}\n".format(lyr.properties["name"], oid, status)
                    # else:
            ...
            for record in rel_records:
                try:
                    ...
                    if 'error' in response:
                        ...
                        error_messages += 'Error accessing comment table {}\n'.format(relname)
                        # else:
                        ...
                        error_messages += "Status of updates to {}, ObjectID:{} comments: {}\n".format(relname, rel_oid, status)
                        # else:
        ...
        if log_to_file:
            ...
            error_messages += 'error: {} {}, Line {}\n'.format(exc_typ, str(ex), exc_tb.tb_lineno)
        ...
        if log_to_file:
            ...
            error_messages += 'error: {}, Line {}\n'.format(exc_typ, exc_tb.tb_lineno)
...
# Send email with errors
        Send('Sync SR data from AGOL to CW Script')
        print ("email sent")

def Send(Title):
    SUBJECT = Title
    FROM = 'steven_cotterill@washingtoncountyor.gov'
    TO = 'steven_cotterill@washingtoncountyor.gov'

    if len(error_messages)>0:

        BODY = str.join('\r\n', ("From: {fname}".format(fname=FROM),
               "To: {tname}".format(tname=TO),
               "Subject: *** ERRORS *** {sname}".format(sname=SUBJECT) ,
               "",
               error_messages))
...
if __name__ == "__main__":
    ...
    configfile = r"\\pwebgisapp1\GIS_Process_Scripts\PRODpyScriptShare\LUTOPS\Cityworks_ConnectSync_withCPRapp_config.json"
